...
def download_wikipedia(path:str):
  ...
  if getenv("TRAIN_DOWNLOAD", 0):
    ...
    gdrive_download("https://drive.google.com/u/0/uc?id=1dy_Jt3s6CYy6SqAJeIveI1P2msSNlhBH", os.path.join(path, "results_text.zip"))
    wikipedia_uncompress_and_extract(os.path.join(path, "results_text.zip"), path)
    if getenv("VERIFY", 0):
